...
if [[ -e /etc/php/${PHP_VERSION}/mods-available/tideways.ini ]]; then
    cp -a /etc/php/${PHP_VERSION}/mods-available/tideways.ini \
    ...
    printf "tideways.udp_connection=\"tideways:8135\"\ntideways.connection=\"tcp://tideways:9135\"\ntideways.monitor_cli=1\n" \
        >> /etc/php/${PHP_VERSION}/mods-available/tideways.ini
...
if [[ -e /etc/php/${PHP_VERSION}/mods-available/blackfire.ini ]]; then
    sed -e 's#\(blackfire.agent_socket\).*#\1=tcp://blackfire:8707#' \
        -i /etc/php/${PHP_VERSION}/mods-available/blackfire.ini
fi
